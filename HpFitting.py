...
class HpFitting:
    ...
    def execute_pipeline(self,df):
        ...
        fig.update_layout(title='Reduced Polytropic Head for Fitted Cases',xaxis_title='Reduced Flow (Qr)',yaxis_title='Reduced Head (Hpr)',template='plotly_white',xaxis=dict(range=[0, Qr_max_bound*1.15]),yaxis=dict(range=[0, np.max(Y_fine)*1.15]))#,legend=dict(yanchor="bottom", y=0.99, xanchor="right", x=0.99))
