...
class PFitting:
    ...
    def run_calibrations(self,df,QrHpScaleFtr):
        ...
        fig.update_layout(title='Reduced Power for Fitted Cases',xaxis_title='Reduced Flow (Qr)',yaxis_title='Reduced Power (Pr)',template='plotly_white',xaxis=dict(range=[0, Qr_max_bound*1.15]),yaxis=dict(range=[0, np.max(Y_fine)*1.15]))
